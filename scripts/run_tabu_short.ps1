# Run Tabu with reduced iterations across all instances in data/
# Usage: Open PowerShell in repository root and run: .\scripts\run_tabu_short.ps1
...
foreach ($inst in $instances) {
    ...
    foreach ($seed in $seeds) {
        Write-Host "Running Tabu on $($inst.Name) seed=$seed iterations=$iterations"
        $stdin = @"
non
$($inst.FullName)
tabu
...
"@
        ...
        $logFile = "logs/tabu_${stem}_${seed}.log"
...
Write-Host "All Tabu jobs finished." 
